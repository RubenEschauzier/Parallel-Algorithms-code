...
int main( int argc, char **argv ) {
	...
	mcbsp_util_address_map_initialize( &map );
	...
	mcbsp_util_address_table_initialize( &table, 5 );
	if( table.cap != 16 || table.table == NULL || table.P != 5 ) {
		fprintf( stderr, "FAILURE \t initialising the mcbsp_util_address_table failed!\n" );
		exit( EXIT_SUCCESS );
	}

	mcbsp_util_address_table_grow( &table, 32 );
	if( table.cap != 64 || table.table == NULL || table.P != 5 ) {
		(void) fprintf( stderr, "FAILURE \t growing the mcbsp_util_address_table failed! Cap = %zd, raw memory @ %p, P = %zd\n", table.cap, (void*)(table.table), table.P );
		exit( EXIT_SUCCESS );
	}
	...
	mcbsp_util_stack_initialize( &stack, sizeof( unsigned long int ) );
	...
	mcbsp_util_stack_grow( &stack, 32 );
