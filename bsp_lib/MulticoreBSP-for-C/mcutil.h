 ...
 *  -# mcbsp_util_address_table_initialize
 *  -# mcbsp_util_address_table_destroy
 * These functions are \em not thread-safe.
 *
 * There are three modifier functions:
 *  -# mcbsp_util_address_table_setsize (not thread-safe)
 *  -# mcbsp_util_address_table_grow (not thread-safe)
 *  -# mcbsp_util_address_table_set (thread-safe)
 * 
 * There is one accessor function:
 *  -# mcbsp_util_address_table_get
 * This function is thread-safe only if no other threads
 * execute resource management or modifier functions on
 * the same table.
 */
 ...
 */
extern const struct mcbsp_util_address_map MCBSP_UTIL_EMPTY_ADDRESS_MAP;
...
/**
 * Increases the capacity of a given stack to match or exceed a
 * given target capacity. Uses recursive doubling.
 *
 * @param[in] stack       the stack whose capacity to increase.
 * @param[in] target_size the minimum size at function exit.
 */
...
/**
 * Increases the capacity of a given address table to match at least
 * a given capacity.
 *
 * @param[in] table       Pointer to the struct of the table to
 *                        increase the storage of.
 * @param[in] target_size the minimum size at function exit.
 *
 * \warning This function is \em not thread-safe.
 */
 ...
 *
 * @param table Pointer to the struct to check the size of, and expand the size of, if necessary.
 ...
 *
 * \warning This function is costly to call especially
 *          when many function do so concurrently, using
 *          the same table.
 */
void mcbsp_util_address_table_set(
	struct mcbsp_util_address_table * const table,
	const size_t key,
 ...
 *
 * This function is thread-safe as long as other
 * threads only access the same \a table.
 */
const struct mcbsp_util_address_table_entry * mcbsp_util_address_table_get(
	const struct mcbsp_util_address_table * const table,
	const size_t key,
	const size_t version
);
