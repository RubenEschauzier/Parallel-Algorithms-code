...
const struct mcbsp_util_address_map MCBSP_UTIL_EMPTY_ADDRESS_MAP = { 0, 0, NULL, NULL };
...
struct mcbsp_util_machine_info *mcbsp_util_createMachineInfo( void ) {
			...
			if( sscanf( LINE_BUFFER, "%s %[^\n]", key, RECURSIVE_LINE_BUFFER ) == 2 ) {
				...
				if( !MCBSP_MACHINE_INFO->Cset && strcmp( key, "cores" ) == 0 ) {
					...
					if( sscanf( LINE_BUFFER, "%s %lu", key, &value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parsing error while processing `cores' key in machine.info at: %s", LINE_BUFFER );
						assert( Cset == false );
					} else {
				...
				//check for threads key
					...
					unsigned long int value = 0;
					if( sscanf( LINE_BUFFER, "%s %lu", key, &value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parsing error while processing `threads' key in machine.info at: %s", LINE_BUFFER );
						assert( Tset == false );
					} else {
				...
				//check for threads_per_core key
					...
					bool success = true;
					if( sscanf( LINE_BUFFER, "%s %lu", key, &value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parsing error while processing `threads_per_core' key (reverting to default) in machine.info at: %s",
							LINE_BUFFER );
						success = false;
					} else {
				...
				//check for unused_threads_per_core key
					...
					bool success = true;
					if( sscanf( LINE_BUFFER, "%s %lu", key, &value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parsing error while processing `unused_threads_per_core' key in machine.info at: %s", LINE_BUFFER );
						success = false;
					} else {
				...
				//check for cp_f (checkpointing frequency)
					...
					if( sscanf( LINE_BUFFER, "%s %zu", key, &value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parse error while processing `checkpoint_frequency' key in machine.info (at line %s", LINE_BUFFER );
						success = false;
					} else {
				...
				//check for safe_cp_f (checkpointing frequency when failures are imminent)
					...
					bool success = true;
					if( sscanf( LINE_BUFFER, "%s %zu", key, &value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parse error while processing `safe_checkpoint_frequency' key in machine.info (at line %s", LINE_BUFFER );
						success = false;
					} else {
				...
				//check for threads_numbering key
					...
					if( sscanf( LINE_BUFFER, "%s %s", key, value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parsing error while processing `thread_numbering' key (reverting to default) in machine.info at: %s", LINE_BUFFER );
						success = false;
					} else {
						if( strcmp( value, "consecutive" ) == 0 ) {
							...
							(void) fprintf( stderr, "Warning: unkown value for the `thread_numbering' key in machine.info (%s); reverting to default value.\n", value );
							success = false;
						}
					}
					if( !success ) {
				...
				//check for affinity key
					...
					bool success = true;
					if( sscanf( LINE_BUFFER, "%s %s", key, value ) != 2 ) {
						(void) fprintf( stderr, "Warning: parsing error while processing `affinity' key in machine.info (reverting to default) at: %s", LINE_BUFFER );
						success = false;
					} else {
						if( strcmp( value, "scatter" ) == 0 ) {
							...
							(void) fprintf( stderr, "Warning: unkown value for the `affinity' key in machine.info (%s); reverting to default value.\n", value );
							success = false;
						}
					}
				...
				//check for pinning key
					...
					do {
						...
						if( number_read < 1 ) {
							(void) fprintf( stderr, "Warning: error in parsing a value for `pinning' keyword in machine.info at: %s\n", RECURSIVE_LINE_BUFFER );
							success = false;
						} else {
					...
					if( cur < MCBSP_MACHINE_INFO->threads ) {
						(void) fprintf( stderr, "Warning: not enough values for `pinning' keyword (read %zd) in machine.info at: %s", 
							cur, LINE_BUFFER );
						success = false;
					} else if( cur > MCBSP_MACHINE_INFO->threads ) {
						(void) fprintf( stderr, "Warning: too many values for `pinning' keyword in machine.info (remainder will be ignored) at: %s", LINE_BUFFER );
					}
					//if reading of the pinning list failed, fall back to sane defaults
					if( !success ) {
						(void) fprintf( stderr, "Warning: setting manual pinning failed, reverting to default affinity strategy if necessary.\n" );
					...
					if( MCBSP_MACHINE_INFO->num_reserved_cores == 0 ) {
						(void) fprintf( stderr, "Warning: number of reserved cores (%zd) does not make sense.\n",
							MCBSP_MACHINE_INFO->num_reserved_cores
						 );
						success = false;
					}
					...
					if( success ) {
						...
						do {
							number_read = sscanf( RECURSIVE_LINE_BUFFER, "%lu %[^\n]", &value, RECURSIVE_LINE_BUFFER );
							if( number_read < 1 ) {
								(void) fprintf( stderr, "Warning: error in parsing a value for `reserved_cores' keyword in machine.info at: %s\n", RECURSIVE_LINE_BUFFER );
								success = false; 
							} else {
					...
					(void) fprintf( stderr, "Warning: unknown key in machine.info (%s)\n", key );
				}
			} else {
				(void) fprintf( stderr, "Warning: error while reading machine.info at: %s", LINE_BUFFER );
			}
		} //end while-not-(EOF-or-I/O-error) loop
		//close file handle
		if( fclose( fp ) != 0 ) {
			(void) fprintf( stderr, "Warning: error while closing the machine.info file." );
		}
	...
	if( MCBSP_MACHINE_INFO->cores <= MCBSP_MACHINE_INFO->num_reserved_cores ) {
		(void) fprintf( stderr,
			"Warning: number of reserved cores (%zd) is larger than or equal to the number of available cores (%zd). Attempting to continue anyway...\n",
			MCBSP_MACHINE_INFO->num_reserved_cores,
			MCBSP_MACHINE_INFO->cores
		);
	}
	if( MCBSP_MACHINE_INFO->threads_per_core <= MCBSP_MACHINE_INFO->unused_threads_per_core ) {
		(void) fprintf( stderr,
			"Warning: number of unused threads per core (%zd) leaves no available threads (since threads_per_core equals %zd). Attempting to continue anyway...\n",
			MCBSP_MACHINE_INFO->unused_threads_per_core,
			MCBSP_MACHINE_INFO->threads_per_core
		);
	}
	if( (MCBSP_MACHINE_INFO->cores - MCBSP_MACHINE_INFO->num_reserved_cores) * 
		(MCBSP_MACHINE_INFO->threads_per_core - MCBSP_MACHINE_INFO->unused_threads_per_core) < MCBSP_MACHINE_INFO->threads ) {
		(void) fprintf( stderr,
			"Warning: number of threads (%zd) is set higher than the number of available cores (%zd-%zd) times the number of available threads per core (%zd-%zd).",
			MCBSP_MACHINE_INFO->threads,
			MCBSP_MACHINE_INFO->cores,
			MCBSP_MACHINE_INFO->num_reserved_cores,
			MCBSP_MACHINE_INFO->threads_per_core,
			MCBSP_MACHINE_INFO->unused_threads_per_core
		);
		(void) fprintf( stderr, " Attempting to continue anyway...\n" );
	}
	//done
...
size_t mcbsp_util_sort_unique_integers( size_t * const array, const size_t length, const size_t lower_bound, const size_t upper_bound ) {
	...
	//sanity checks
	if( array == NULL ) {
		(void) fprintf( stderr, "Error: NULL array passed to mcbsp_util_sort_unique_integers!\n" );
		mcbsp_util_fatal();
	}
	if( lower_bound >= upper_bound ) {
		(void) fprintf( stderr, "Error: invalid vazde range passed to mcbsp_util_sort_unique_integers (%zd,%zd)!\n",
	...
	for( size_t s = 0; s < length; ++s ) {
		...
		if( value < lower_bound || value >= upper_bound ) {
			(void) fprintf( stderr, "Error: array to sort contained out-of-range element (mcbsp_util_sort_unique_integers in mcutil.c)!\n" );
			mcbsp_util_fatal();
		} else {
			const size_t toUpdate = value - lower_bound;
			if( !warned && counting_sort[ toUpdate ] ) {
				(void) fprintf( stderr, "Warning: array to sort contains duplicate values (mcbsp_util_sort_unique_integers in mcutil.c)!\n" );
				warned = true;
			} else {
...
bool mcbsp_util_contains( const size_t * const array, const size_t value, const size_t lo, const size_t hi ) {
	//sanity
	if( lo >= hi ) {
		(void) fprintf( stderr, "Error: bounds given to mcbsp_util_contains (mcutil.c) do not make sense (%zd,%zd)!\n",
			lo, hi );
		mcbsp_util_fatal();
	}
...
struct mcbsp_util_pinning_info mcbsp_util_pinning( const size_t P, struct mcbsp_util_machine_info * const machine, const struct mcbsp_util_machine_partition * const super_machine ) {
	...
	if( P == 0 ) {
		mcbsp_util_stack_initialize( &(ret.partition), sizeof( struct mcbsp_util_machine_partition ) );
		return ret;
	}
	...
	if( P > available_threads ) {
		(void) fprintf( stderr, "Warning: %zd threads requested but only %zd available. If this was on purpose, please supply a proper manual affinity and increase the maximum threads usable by MulticoreBSP manually (see the documentation for details)\n", P, available_threads );
	}
	...
	if( machine->affinity == MANUAL ) {
		...
		if( ret.partition.top != P ) {
			(void) fprintf( stderr, "Error: expected %zd parts but mcbsp_util_partition returned only %zd.\n", P, ret.partition.top );
			mcbsp_util_fatal();
		}
...
void mcbsp_util_stack_initialize( struct mcbsp_util_stack * const stack, const size_t size ) {
	stack->cap   = 16;
	stack->top   = 0;
	stack->size  = size;
	stack->array = mcbsp_util_malloc( 16 * size, "mcbsp_util_stack_initialize initial stack array" );
}
...
void mcbsp_util_stack_grow( struct mcbsp_util_stack * const stack, const size_t target_size ) {
	...
	void * replace = mcbsp_util_malloc( new_cap * stack->size, "mcbsp_util_stack_grow replacement array" );
	//copy old contents
	assert( stack->top <= new_cap );
...
void mcbsp_util_stack_push( struct mcbsp_util_stack * const stack, const void * const item ) {
	if( stack->top >= stack->cap ) {
		mcbsp_util_stack_grow( stack, stack->top + 1 );
	}
...
void mcbsp_util_address_table_initialize( struct mcbsp_util_address_table * const table, const size_t P ) {
	...
	table->table = mcbsp_util_malloc( 16 * sizeof( struct mcbsp_util_stack * ), "mcbsp_util_address_table_initialize array of stack pointers" );
	for( unsigned int i = 0; i < 16; ++i ) {
		table->table[ i ] = mcbsp_util_malloc( P * sizeof( struct mcbsp_util_stack ), "mcbsp_util_address_table_initialize array of stacks" );
		for( size_t k = 0; k < table->P; ++k ) {
			mcbsp_util_stack_initialize( &(table->table[ i ][ k ]), sizeof(struct mcbsp_util_address_table_entry) );
		}
	}
#else
	table->table= mcbsp_util_malloc( 16 * sizeof( struct mcbsp_util_address_table_entry * ), "mcbsp_util_address_table_initialize array of entry pointers" );
	for( unsigned int i = 0; i < 16; ++i ) {
		table->table[ i ] = mcbsp_util_malloc( P * sizeof( struct mcbsp_util_address_table_entry ), "mcbsp_util_address_table_initialize array of entries" );
	}
#endif
}
...
void mcbsp_util_address_table_grow( struct mcbsp_util_address_table * const table, const size_t target_size ) {
	...
	(void) fprintf( stderr, "Debug, table_grow: growing %p to at least %zd\n", table, target_size ); fflush(stderr);
#endif
	size_t new_size = table->cap;
	if( new_size == 0 ) {
		new_size = 1;
	}
...
#ifdef MCBSP_ALLOW_MULTIPLE_REGS
	struct mcbsp_util_stack ** replace = mcbsp_util_malloc( new_size * sizeof( struct mcbsp_util_stack * ), "mcbsp_util_address_table_grow array of stack pointers" );
	for( size_t i = 0; i < new_size; ++i ) {
		...
		if( i < table->cap ) {
			...
			for( size_t k = 0; k < table->P; ++k ) {
				mcbsp_util_stack_initialize( &(replace[ i ][ k ]), sizeof( struct mcbsp_util_address_table_entry )  );
			}
		}
	}
#else
	struct mcbsp_util_address_table_entry ** replace = mcbsp_util_malloc( new_size * sizeof( struct mcbsp_util_address_table_entry * ), "mcbsp_util_address_table_grow array of entry pointers" );
	for( size_t i = 0; i < new_size; ++i ) {
	...
	table->cap   = new_size;
#ifdef MCBSP_DEBUG_REGS
	(void) fprintf( stderr, "Debug, table_grow: %p now has capacity %zd (>%zd)\n", table, table->cap, target_size ); fflush(stderr);
#endif
}

void mcbsp_util_address_table_setsize( struct mcbsp_util_address_table * const table, const size_t target_size ) {
	//if capacity is good enough, exit before getting a lock
	if( table->cap > target_size ) {
		return;
	}

	mcbsp_util_address_table_grow( table, target_size );
}
...
void mcbsp_util_address_table_set( struct mcbsp_util_address_table * const table, const size_t key, const size_t version, void * const value, const size_t size ) {
	...
	new_entry.size    = size;
	assert( key < table->cap );
	assert( version < table->P );
 #ifdef MCBSP_DEBUG_REGS
	(void) fprintf( stderr, "Debug, tabel_set: address table capacity is %zd, size is %zd, top is %zd\n", stack->cap, stack->size, stack->top );
	fflush(stderr);
 #endif
	if( stack->cap <= stack->size ) {
		mcbsp_util_stack_grow( stack, stack->size + 1 );
 #ifdef MCBSP_DEBUG_REGS
		(void) fprintf( stderr, "Debug, table_set: new address table capacity is %zd with size %zd and top %zd\n", stack->cap, stack->size, stack->top );
		fflush(stderr);
 #endif
	}
	mcbsp_util_stack_push( stack, &new_entry );
 #ifdef MCBSP_DEBUG_REGS
	(void) fprintf( stderr, "Debug, table_set: %zd, %zd at %p has top: %zd\n", key, version, table, stack->top ); fflush(stderr);
 #endif
#else
...
const struct mcbsp_util_address_table_entry * mcbsp_util_address_table_get( const struct mcbsp_util_address_table * const table, const size_t key, const size_t version ) {
#ifdef MCBSP_ALLOW_MULTIPLE_REGS
	struct mcbsp_util_stack * const stack = &( table->table[ key ][ version ] );
...
#ifdef MCBSP_DEBUG_REGS
	(void) fprintf( stderr, "Debug, table_delete: deleting %zd, %zd from %p. Stack size: %zd.\n", key, version, table, table->table[key][version].top );
#endif
#ifdef MCBSP_ALLOW_MULTIPLE_REGS
	struct mcbsp_util_stack * const stack = &( table->table[ key ][ version ] );
	assert( !mcbsp_util_stack_empty( stack ) );
...
void mcbsp_util_address_map_initialize( struct mcbsp_util_address_map * const address_map ) {
#ifndef NDEBUG
	if( address_map == NULL ) {
		(void) fprintf( stderr, "Warning: mcbsp_util_address_map_initialize called with NULL argument.\n" );
		return;
	}
#endif
	...
	address_map->cap    = 16;
	address_map->size   = 0;
	address_map->keys   = mcbsp_util_malloc( address_map->cap * sizeof( void * ), "mcbsp_util_address_map_initialize key array" );
	address_map->values = mcbsp_util_malloc( address_map->cap * sizeof( size_t ), "mcbsp_util_address_map_initialize value array" );
}

void mcbsp_util_address_map_grow( struct mcbsp_util_address_map * const address_map ) {
#ifndef NDEBUG
	if( address_map == NULL ) {
		(void) fprintf( stderr, "Warning: mcbsp_util_address_map_grow called with NULL argument.\n" );
		return;
	}
#endif

	//compute new capacity
	size_t new_cap = 2 * address_map->cap;
	if( address_map->cap == 0 ) {
		 new_cap = 1;
	}

	//create replacement arrays
	void * * replace_k = mcbsp_util_malloc( new_cap * sizeof( void * ), "mcbsp_util_address_map_grow key array" );
	size_t * replace_v = mcbsp_util_malloc( new_cap * sizeof( size_t ), "mcbsp_util_address_map_grow value array" );
...
void mcbsp_util_address_map_destroy( struct mcbsp_util_address_map * const address_map ) {
	//delete arrays
	if( address_map->keys != NULL ) {
		free( address_map->keys   );
	}
	if( address_map->values != NULL ) {
		free( address_map->values );
	}
...
#ifndef MCBSP_ALLOW_MULTIPLE_REGS
			(void) fprintf( stderr, "Warning: mcbsp_util_address_map_insert ignored as key already existed (at %zd/%zd)!\n", insertionPoint, (address_map->size-1) );
#endif
			return; //key was already here; this is either an error, or we're trivially done
...
void mcbsp_util_address_map_remove( struct mcbsp_util_address_map * const address_map, void * const key ) {
	...
	if( address_map->size == 0 ) {
		(void) fprintf( stderr, "Warning: mcbsp_util_address_map_remove ignored since map was empty!\n" );
	...
	if( address_map->keys[ index ] != key ) {
		(void) fprintf( stderr, "Warning: mcbsp_util_address_map_remove ignored since key was not found!\n" );
...
void mcbsp_util_check_machine_info( struct mcbsp_util_machine_info * const machine ) {
	...
	if( machine->cores <= machine->num_reserved_cores ) {
		(void) fprintf( stderr, "Error: there are no free cores for MulticoreBSP to run on!\n" );
		mcbsp_util_fatal();
	}
	...
	if( available_threads < machine->threads && !(machine->Tset) ) {
		(void) fprintf( stderr, "Warning: machine information shows more threads (%zd) than #available_cores (%zd) times #threads_per_core (%zd). Attempting to continue anyway...\n",
				machine->threads, machine->cores, machine->threads_per_core );
	}
...
struct mcbsp_util_machine_partition mcbsp_util_derive_partition( const struct mcbsp_util_machine_info * const info ) {
	...
	(void) printf( "Given machine info: %zd threads, %zd cores, %zd threads_per_core, %zd reserved cores, %zd reserved threads_per_core.\n",
		(info->threads),
		(info->cores),
	...
	case COMPACT:
		...
		break;
	default:
		(void) fprintf( stderr, "Warning: unrecognised affinity in mcbsp_util_derive_partition!\n" );
		//assume manual partitioning via
		//fall through
	case MANUAL:
...
#ifdef MCBSP_DEBUG_AFFINITY
	(void) printf( "Returning partition %zd-%zd/%zd.\n", ret.start, ret.end, ret.stride );
#endif
...
struct mcbsp_util_stack mcbsp_util_partition( const struct mcbsp_util_machine_partition subpart, const size_t P, const enum mcbsp_affinity_mode affinity ) {
	...
	mcbsp_util_stack_initialize( &ret, sizeof( struct mcbsp_util_machine_partition ) );
	...
	for( size_t s = 0; s < P; ++s ) {
		...
		switch( affinity ) {
			...
			case MANUAL:
			{
				(void) fprintf( stderr, "Warning: mcbsp_util_partition cannot derive subpartitions when affinity is in manual mode. This function should not have been called!\n" );
				return ret;
			}
			default:
			{
				(void) fprintf( stderr, "Error: undefined affinity in mcbsp_util_partition. Attempting to continue anyway.\n" );
				return ret;
			}
		}
...
void mcbsp_util_translate_standard_pinning( const struct mcbsp_util_machine_info * const info, size_t * const array, const size_t P ) {
	...
	if( info->thread_numbering != WRAPPED ) {
		(void) fprintf( stderr, "Error: unrecognised thread numbering in mcbsp_util_translate_standard_pinning! Aborting...\n" );
		mcbsp_util_fatal();
	}
...
#ifndef NDEBUG
	const void * const dest_end = ((char*)destination) + size;
	assert( (destination < source && dest_end < source) ||
		(destination > source && dest_end > source) );
#endif
...
void * mcbsp_util_malloc( const size_t size, const char * const name ) {
	...
	bool fail = false;
#if !defined _WIN32 && !defined __ANDROID__ && ((defined _POSIX_C_SOURCE && _POSIX_C_SOURCE >= 200112L) || defined _DARWIN_C_SOURCE)
	//do allocation using POSIX memalign, if supported
	const int rc = posix_memalign( &ret, MCBSP_ALIGNMENT, size );
	if( rc != 0 ) {
		fail = true;
	}
#elif defined _WIN32
	//windows has their own aligned alloc function...
