...
void mcbsp_profiling_set_description( const char * const name ) {
	...
	struct mcbsp_thread_data * const tdata = pthread_getspecific( mcbsp_internal_thread_data );
	//set name
	(void) strncpy( tdata->superstep_stats.name, name, MCBSP_SUPERSTEP_STATS_NAME_MAX-1 );
	//truncate if necessary
	tdata->superstep_stats.name[ MCBSP_SUPERSTEP_STATS_NAME_MAX-1 ] = '\0';
}
...
void* mcbsp_internal_spmd( void *p ) {
	...
	data = mcbsp_internal_initialize_thread_data( data );
...
struct mcbsp_thread_data * mcbsp_internal_initialize_thread_data( struct mcbsp_thread_data * const data ) {
	//allocate new thread-local data
	struct mcbsp_thread_data *thread_data = mcbsp_util_malloc( sizeof( struct mcbsp_thread_data ), "mcbsp_internal_initialize_thread_data thread data struct" );
	...
	*thread_data = *data;
	...
	thread_data->request_queues = mcbsp_util_malloc( P * sizeof( struct mcbsp_util_stack ), "mcbsp_internal_initialize_thread_data communication request stack array" );
	thread_data->queues         = mcbsp_util_malloc( P * sizeof( struct mcbsp_util_stack ), "mcbsp_internal_initialize_thread_data communication stack array" );
	thread_data->hpsend_queues  = mcbsp_util_malloc( P * sizeof( struct mcbsp_util_stack ), "mcbsp_internal_initialize_thread_data hp-communication stack array" );
	//initialise stacks
	for( size_t i = 0; i < P; ++i ) {
		mcbsp_util_stack_initialize( &(thread_data->request_queues[ i ]), sizeof( struct mcbsp_get_request ) );
		mcbsp_util_stack_initialize( &(thread_data->queues[ i ]),         sizeof( struct mcbsp_message) );
		mcbsp_util_stack_initialize( &(thread_data->hpsend_queues[ i ]),  sizeof( struct mcbsp_hpsend_request) );
	}

	//initialise local to global map
	mcbsp_util_address_map_initialize( &(thread_data->local2global ) );

	//initialise stack used for hp communication
	mcbsp_util_stack_initialize( &(thread_data->hpdrma_queue), sizeof( struct mcbsp_hp_request ) );

	//initialise BSMP queue
	mcbsp_util_stack_initialize( &(thread_data->bsmp), sizeof( size_t ) );

	//initialise profile vector
	mcbsp_util_stack_initialize( &(thread_data->profile), sizeof( struct mcbsp_superstep_stats ) );

	//initialise stack used for efficient registration of globals after de-registrations
	mcbsp_util_stack_initialize( &(thread_data->removedGlobals), sizeof( unsigned long int ) );

	//initialise stack used for de-registration of globals
	mcbsp_util_stack_initialize( &(thread_data->localsToRemove), sizeof( void * ) );

	//initialise push request queue
	mcbsp_util_stack_initialize( &(thread_data->localsToPush), sizeof( struct mcbsp_push_request ) );

	//initialise the map-search stack
	mcbsp_util_stack_initialize( &(thread_data->globalsToPush), sizeof( unsigned long int ) );
...
void mcbsp_internal_print_profile( const struct mcbsp_init_data * const init ) {
	...
	for( size_t i = 0; i < N; ++i ) {
		...
		double true_bsp_time = p0.communication + p0.buffering;
		...
		for( size_t k = 1; k < P; ++k ) {
			...
			if( true_bsp_time < pk.communication + pk.buffering ) {
				true_bsp_time = pk.communication + pk.buffering;
			}
		}
		...
		printf( "maximum time spent in pure computations   = %lf seconds\n", true_compute_time );
		printf( "maximum time spent in communication phase = %lf seconds\n", p0.communication );
		printf( "maximum time spent in BSP calls           = %lf seconds\n", true_bsp_time );
		...
		total_compute_time += true_compute_time;
		total_bsp_time += true_bsp_time;
	}

	//print footer
	printf(
		"\nBSP signature (time spent in pure computations divided by the total runtime): %lf / %lf = %lf.\n",
		total_compute_time, (total_compute_time + total_bsp_time), total_compute_time / (total_compute_time + total_bsp_time)
	);
