...
void MCBSP_FUNCTION_PREFIX(begin)( const bsp_pid_t P ) {
	...
	if( init == NULL ) {
		...
		if( thread_data == NULL ) {
			...
			if( init == NULL ) {
				(void) fprintf( stderr, "Could not perform an implicit initialisation!\n" );
				mcbsp_util_fatal();
			}
	...
	mcbsp_util_address_table_initialize( &(init->global2local), ((size_t)P) );
	...
	//sanity checks
	if( pinning == NULL ) {
		(void) fprintf( stderr, "Could not get a valid pinning!\n" );
		mcbsp_util_fatal();
	}
...
#ifdef __ANDROID__
 #pragma message "Pinning is not yet implemented on Android"
#endif
			...
			thread_data = mcbsp_internal_initialize_thread_data( thread_data );
...
void MCBSP_FUNCTION_PREFIX(sync)( void ) {
	...
	(void) printf( "MCBSP_DEBUG_REGS: deregistering %zd registrations...\n", data->localsToRemove.top );
#endif
	...
	for( size_t i = 0; i < data->localsToRemove.top; ++i ) {
...
#ifdef MCBSP_DEBUG_REGS
			found_local = false;
			(void) fprintf( stderr, "Debug: %zu needs to consult remote processes to find i-th address.\n", data->bsp_id );
#endif
			//use other process' logic to process this pop_reg
			size_t s = 0;
			void * const * array2 = NULL;
			//find processor with non-NULL entry at entry i
			for( ; s < data->init->P; ++s ) {
				if( i >= data->init->threadData[ s ]->localsToRemove.top ) {
					(void) fprintf( stderr, "Warning: not all processes deregistered the same number of addresses!\n" );
					continue;
				}
...
#endif
			//otherwise, use this remote processor's logic
...
#ifdef MCBSP_DEBUG_REGS
		(void) fprintf( stderr, "Debug: %zu removes address %p (global table entry %zd).", data->bsp_id, search_address, globalIndex );
		if( found_local ) {
			(void) fprintf( stderr, " This global ID was found locally.\n" );
		} else {
			(void) fprintf( stderr, " This global ID was found by querying a remote process.\n" );
		}
		fflush(stderr);
#endif
		...
		if( globalIndex == SIZE_MAX ) {
			(void) fprintf( stderr, "Warning: tried to de-register a non-registered variable. The corresponding call to bsp_pop_reg will have no effect.\n" );
		} else {
#ifdef MCBSP_DEBUG_REGS
			(void) fprintf( stderr, "Debug: %zu calls table_delete( %p, %zd, %zu )\n", data->bsp_id, &(data->init->global2local), globalIndex, data->bsp_id );
			fflush(stderr);
#endif
			...
			if( !mcbsp_util_address_table_delete( &(data->init->global2local), globalIndex, data->bsp_id ) ) {
				...
				//delete from map after the next barrier
			}
		}

#ifdef MCBSP_DEBUG_REGS
		(void) fprintf( stderr, "Debug: %zu has local registration count %ld and a removedGlobals stack size of %ld.\n", data->bsp_id, data->localC, data->removedGlobals.top );
#endif
	}
...
#ifdef MCBSP_DEBUG_REGS
		fprintf( stderr, "Debug: %zu removes address %p from local2global map.\n", data->bsp_id, toRemove );
#endif
	...
	//keeps track of any new globals that are added during this superstep
	struct mcbsp_util_address_map new_addresses = MCBSP_UTIL_EMPTY_ADDRESS_MAP;
	...
	for( size_t i = 0; i < data->localsToPush.top; ++i ) {
		...
		//check if this address was newly added earlier during handling this superstep
		const size_t already_added_number = mcbsp_util_address_map_get( &new_addresses, address );
		size_t global_number = already_added_number;
		bool new_address = false;

#ifdef MCBSP_DEBUG_REGS
		if( already_added_number != SIZE_MAX ) {
			(void) fprintf( stderr, "Debug: %zu found address %p was already processed this superstep and was assigned ID %zd.\n", data->bsp_id, address, global_number );
		}
#endif

		//if not, search for the key either locally or remotely:
		if( global_number == SIZE_MAX ) {
			...
			if( address == NULL ) {
				...
				for( ; s < data->init->P; ++s ) {
					if( i >= data->init->threadData[ s ]->localsToPush.top ) {
						(void) printf( "Warning: not all processes registered the same number of memory regions!\n" );
						continue;
					}
					array2 = (struct mcbsp_push_request *)data->init->threadData[ s ]->localsToPush.array;
					if( array2[ i ].address != NULL ) {
						break;
					}
				}
				...
				if( s == data->init->P ) {
					...
					(void) fprintf( stderr, "Warning: tried to register a NULL-address on all processes!\n" );
					continue;
				}
			...
			global_number = mcbsp_util_address_map_get( search_map, search_address );

#ifdef MCBSP_DEBUG_REGS
			(void) fprintf( stderr, "Debug: %zu found address %p to map to global ID %zd. Map size was %zd.\n", data->bsp_id, address, global_number, search_map->size );
#endif
		}

		//if the key was not found, create a new global entry
		//but take care to re-use a previously freed key if one exists
		if( global_number == SIZE_MAX ) {
			new_address = true;
			if( mcbsp_util_stack_empty( &(data->removedGlobals) ) ) {
				global_number = (data->localC)++;
			} else {
				global_number = *(size_t*)mcbsp_util_stack_pop( &(data->removedGlobals) );
			}
#ifdef MCBSP_DEBUG_REGS
			(void) fprintf( stderr, "Debug: %zu assigned address %p a new global ID %zd.\n", data->bsp_id, address, global_number );
#endif
		}

		//if we hadn't seen this address before and the address is new, add it to new_addresses
		assert( global_number != SIZE_MAX );
		if( new_address && already_added_number == SIZE_MAX ) {
			mcbsp_util_address_map_insert( &new_addresses, address, global_number );
		}

#ifdef MCBSP_DEBUG_REGS
		(void) fprintf( stderr, "Debug: %zu adds %p to global ID %zd in address table.\n", data->bsp_id, address, global_number );
		(void) fprintf( stderr, "Debug: %zu calls table_set( %p, %zd, %zu, %p, %zd );\n", data->bsp_id, &(data->init->global2local), global_number, data->bsp_id, address, size );
		fflush(stderr);
#endif
...
#ifdef MCBSP_DEBUG_REGS
		(void) fprintf( stderr, "Debug: %zu added %p to global ID %zd in address table.\n", data->bsp_id, address, global_number );
		fflush(stderr);
#endif
		...
		if( new_address ) {
#ifdef MCBSP_DEBUG_REGS
			(void) fprintf( stderr, "Debug: %zu adds %zd to globalsToPush\n", data->bsp_id, global_number );
#endif
	...
	mcbsp_util_address_map_destroy( &new_addresses );

	//update tagsize, phase 2 (check)
#ifndef MCBSP_NO_CHECKS
	if( data->newTagSize != data->init->tagSize ) {
		(void) fprintf( stderr, "Different tag sizes requested from different processes (%zd requested while process 0 requested %zd)!\n",
			(size_t)(data->newTagSize),
			(size_t)(data->init->tagSize)
		);
		mcbsp_util_fatal();
	}
#endif
	...
	while( !mcbsp_util_stack_empty( &(data->localsToPush) ) ) {
		...
		const size_t global_number = *(size_t*)mcbsp_util_stack_pop( &(data->globalsToPush) );

#ifdef MCBSP_DEBUG_REGS
		(void) fprintf( stderr, "Debug: %zu maps address %p to global ID %lu.\n", data->bsp_id, address, global_number );
#endif
...
#if MCBSP_MODE == 3
	(void) ++(data->superstep_stats.non_communicating);
#endif
...
#ifdef MCBSP_DEBUG_REGS
	fprintf( stderr, "Debug: %zu registers address %p with size %lu.\n", data->bsp_id, address, (unsigned long int)(size_in) );
#endif
...
#ifdef MCBSP_DEBUG_REGS
	(void) fprintf( stderr, "Debug: %zu de-registers address %p.\n", data->bsp_id, address );
#endif
...
void MCBSP_FUNCTION_PREFIX(get)( const bsp_pid_t pid, const void * const source,
	...
	const size_t globalIndex = mcbsp_util_address_map_get( &(data->local2global), source );
#ifndef MCBSP_NO_CHECKS
	if( globalIndex == SIZE_MAX ) {
		(void) fprintf( stderr, "Error: bsp_get into unregistered memory area (%p) requested!\n", source );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}	
#endif
...
#ifndef MCBSP_NO_CHECKS
	if( entry == NULL ) {
		(void) fprintf( stderr, "Error: bsp_get called with an erroneously registered source variable!\n" );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
	if( offset + size > entry->size ) {
		(void) fprintf( stderr, "Error: bsp_get would go out of bounds at source processor (offset=%zd, size=%zd, while registered memory area is %zd bytes)!\n", offset, size, entry->size );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
#endif
	//record source
...
#ifndef MCBSP_NO_CHECKS
	if( entry == NULL ) {
		(void) fprintf( stderr, "Error: bsp_direct_get called with an erroneously registered source variable (address %p was never registered remotely)!\n", source );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
	if( offset + size > entry->size ) {
		(void) fprintf( stderr, "Error: bsp_direct_get would go out of bounds at source processor (offset=%zd, size=%zd, while registered memory area is %zd bytes)!\n", offset, size, entry->size );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
#endif
...
void MCBSP_FUNCTION_PREFIX(hpput)( const bsp_pid_t pid, const void * const source,
	...
	if( offset + size > entry->size ) {
		(void) fprintf( stderr, "Error: bsp_hpput would go out of bounds at destination processor (offset=%zd, size=%zd, while registered memory area is %zd bytes)!\n", offset, size, entry->size );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments.\n" );
	}
	if( entry->address == NULL ) {
		(void) fprintf( stderr, "Error: communication attempted on NULL-registered variable!\n" );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments.\n" );
	}
	if( source == NULL ) {
		(void) fprintf( stderr, "Error: communication of NULL memory address requested.\n" );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments.\n" );
	}
#endif
...
void MCBSP_FUNCTION_PREFIX(hpget)(
	...
	if( (size_t)pid >= data->init->P ) {
		bsp_abort( "Error: bsp_hpget called with an out-of-range PID argument (%ld, while bsp_nprocs()==%zd)!\n",
			(long int)pid, (size_t)(data->init->P) );
	}
	if( destination == NULL ) {
...
#ifndef MCBSP_NO_CHECKS
	if( entry == NULL ) {
		(void) fprintf( stderr, "Error: bsp_hpget called with an erroneously registered source variable!\n" );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
	if( offset + size > entry->size ) {
		(void) fprintf( stderr, "Error: bsp_hpget would go out of bounds at source processor (offset=%zd, size=%zd, while registered memory area is %zd bytes)!\n", offset, size, entry->size );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
	if( entry->address == NULL ) {
		(void) fprintf( stderr, "Error: bsp_hpget requested from NULL-registered remote source!\n" );
		bsp_abort( "Aborting due to BSP primitive call with invalid arguments." );
	}
#endif
